import { defineAgent, defineWorkflow, type FlueHarness, type JsonValue } from '@flue/runtime';
import { local } from '@flue/runtime/node';
import * as v from 'valibot';

// CLI-only, read-only workflow. Intended for local or GitHub Actions use via:
// flue run deep-verify --target node --input '{"repo":"adewale/project"}'
...
type DeepVerifyEnv = Record<string, any>;

const workflowEnvs = new Map<string, DeepVerifyEnv>();
...
const verifierAgent = defineAgent<DeepVerifyEnv>(({ id, env }) => {
...
export default defineWorkflow({
	agent: verifierAgent,
	input: PayloadSchema,
	async run({ harness, input: parsedPayload }) {
		...
		try {
			...
			if (!metadata.pushed_at || metadata.pushed_at < CUTOFF) {
				return {
					...
					repo,
					skipped: true,
					reason: `Repository has not changed since ${CUTOFF}`,
